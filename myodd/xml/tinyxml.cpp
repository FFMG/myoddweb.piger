...
bool TiXmlDocument::LoadFile( const TCHAR* _filename, TiXmlEncoding encoding )
{
  ...
  file = TiXmlFOpen( value.c_str (), _T("rb") );	
	if ( file )
	{
		bool result = LoadFile( file, fileEncoding, encoding );
		fclose( file );
		return result;
	}
	else
...
( 
 FILE* file, 
 myodd::files::FileEncode fileEncoding,
 TiXmlEncoding encoding )
{
  ...
  TCHAR* tbuf = tbuf = myodd::files::Byte2Char( buf, length, fileEncoding );
  if ( 0 == tbuf ) 
  {
    delete [] buf;
    SetError( TIXML_ERROR_OPENING_FILE, 0, 0, TIXML_ENCODING_UNKNOWN );
  ...
  data.reserve( length );
	...
	const TCHAR* lastPos = tbuf;

  if ( encoding == TIXML_ENCODING_UNKNOWN )
  {
    // Check for the Microsoft UTF-8 lead bytes.
    if (	*(tbuf+0) && *(tbuf+0) == TIXML_UTF_LEAD_8_0
      && *(tbuf+1) && *(tbuf+1) == TIXML_UTF_LEAD_8_1
      && *(tbuf+2) && *(tbuf+2) == TIXML_UTF_LEAD_8_2 )
    {
  ...
  if( *lastPos == 0xFEFF )
    lastPos++;
	const TCHAR* p = lastPos;

	while( *p ) 
  {
		assert( p < (tbuf+length) );
		if ( *p == 0xa ) 
    {
		...
		else if ( *p == 0xd ) 
    {
			...
			data += (TCHAR)0xa;						// a proper newline
...
bool TiXmlDocument::SaveFileUTF8( const TCHAR* filename ) const
{
	FILE* fp = TiXmlFOpen( filename, _T("w,ccs=UTF-8"));
  if( 0 == fp )
  {
    return false;
  }
  ...
  size_t size   = sizeof(TCHAR);
  size_t count  = _tcslen(xmlcstr);
  size_t actual = fwrite( xmlcstr, size, count, fp );
  fclose( fp );

  //  did we write the right size.
  return (actual == count);
}
...
bool TiXmlDocument::SaveFile( FILE* fp ) const
{
	if ( useMicrosoftBOM ) 
	{
		const TCHAR TIXML_UTF_LEAD_0 = 0xefU;
		const TCHAR TIXML_UTF_LEAD_1 = 0xbbU;
		const TCHAR TIXML_UTF_LEAD_2 = 0xbfU;
...
void TiXmlAttribute::Print( FILE* cfile, int /*depth*/, TIXML_STRING* str ) const
{
	...
	if (value.find ( _T('\"')) == TIXML_STRING::npos) {
		if ( cfile ) {
...
void TiXmlText::CopyTo( TiXmlText* target ) const
{
	TiXmlNode::CopyTo( target );
	target->cdata = cdata;
  target->encodeString = encodeString;
}


bool TiXmlText::Accept( TiXmlVisitor* visitor ) const
{
  bool tmpEncodeString = visitor->encodeString;
  visitor->encodeString = encodeString;
	bool result = visitor->Visit( *this );
  visitor->encodeString = tmpEncodeString;
  return result;
}
